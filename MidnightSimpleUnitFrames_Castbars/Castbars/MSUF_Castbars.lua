...
do
    ...
    local function ManagerOnUpdate(self, elapsed)
        ...
        for frame in pairs(active) do
            if not frame or not frame:IsShown() or not frame.statusBar then
                ...
                -- FAST PATH: Lightweight time-text update every tick (~60fps).
                -- Cost: 1 dObj read + 1 ToPlain + 1 floor comparison. No pcall, no DB access.
                if frame.timeText and frame._msufCastTimeEnabled ~= false and not frame.MSUF_gcdActive and not frame.isEmpower then
                    ...
                    elseif frame.endTime then
                        local endT = ToPlain(frame.endTime) or 0
                        local r = endT - now
                        if r < 0 then r = 0 end
                        MSUF_SetCastTimeText_Dedup(frame, r)
                    end
                end
    ...
    function MSUF_UpdateCastbarFrame(frame, dt, now)
        ...
        -- `now` is passed from the manager; no recalculation needed.
