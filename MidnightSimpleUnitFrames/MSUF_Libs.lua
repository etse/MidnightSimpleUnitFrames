...
-- -----------------------------------------------------------------------------
-- Embedded LibRangeCheck-3.0 (and deps)
-- Needed for MSUF Range Fade (Target/Focus).
-- Safe no-op if another copy is already loaded.
-- -----------------------------------------------------------------------------
do
    local function MSUF_Embed_LibStub()
    ...
    local function MSUF_Embed_CallbackHandler()
    ...
    local function MSUF_Embed_LibRangeCheck()
        --[[
...
lib.harmNoItemsRCInCombat = lib.harmNoItemsRCInCombat or {}
  ...
  runStep()
    ...
    local LibStub = _G.LibStub
    local get = LibStub and LibStub.GetLibrary
    if LibStub and get and not get(LibStub, "CallbackHandler-1.0", true) then
        MSUF_Embed_CallbackHandler()
    end

    if LibStub and get and not get(LibStub, "LibRangeCheck-3.0", true) then
