...
-- PERF: Cache C_UnitAuras function refs once at file scope
local _C_UnitAuras = C_UnitAuras
local _GetAuraSlots = _C_UnitAuras and _C_UnitAuras.GetAuraSlots
local _GetAuraDataBySlot = _C_UnitAuras and _C_UnitAuras.GetAuraDataBySlot
local _GetUnitAuraInstanceIDs = _C_UnitAuras and _C_UnitAuras.GetUnitAuraInstanceIDs
local _GetAuraDataByAuraInstanceID = _C_UnitAuras and _C_UnitAuras.GetAuraDataByAuraInstanceID
local _IsAuraFilteredOut = _C_UnitAuras and _C_UnitAuras.IsAuraFilteredOutByInstanceID
...
local function GetAuraList(unit, filter, onlyPlayer, maxCount, out, entry)
    ...
    if not unit or not _C_UnitAuras then
        return out
    end
...
if entry and onlyPlayer ~= true and type(maxCount) == "number" and maxCount > 0 then
    ...
    -- PERF: use file-scope cached function refs
    local getSlots = _GetAuraSlots
    local getBySlot = _GetAuraDataBySlot
    if type(maxCount) == "number" and maxCount > 0 and type(getSlots) == "function" and type(getBySlot) == "function" then
        ...
        if n > 0 then
            -- PERF: use local counter instead of #out + 1 per iteration
            local outN = 0
            for i = 1, n do
...
-- Legacy fallback: instanceID list (may be large). We cap loop to maxCount when provided.
    local getIDs  = _GetUnitAuraInstanceIDs
    local getData = _GetAuraDataByAuraInstanceID
...
local function MSUF_A2_GetPlayerAuraIdSetCached(entry, unit, filter, maxCount)
    ...
    local _getIDs = _GetUnitAuraInstanceIDs
    if type(_getIDs) ~= "function" then
        entry[keysNField] = 0
        return nil
    end
    local ids = _getIDs(unit, filter .. "|PLAYER")
    if type(ids) ~= "table" then
        ids = _getIDs(unit, "PLAYER|" .. filter)
    elseif #ids == 0 then
        local ids2 = _getIDs(unit, "PLAYER|" .. filter)
