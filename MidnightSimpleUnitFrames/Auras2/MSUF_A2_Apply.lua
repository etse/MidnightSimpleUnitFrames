...
local _GetAuraDataByAuraInstanceID = _C_UnitAuras and _C_UnitAuras.GetAuraDataByAuraInstanceID
local _GetAuraDuration = _C_UnitAuras and _C_UnitAuras.GetAuraDuration
local _DoesAuraHaveExpirationTime = _C_UnitAuras and _C_UnitAuras.DoesAuraHaveExpirationTime
local _GetAuraApplicationDisplayCount = _C_UnitAuras and _C_UnitAuras.GetAuraApplicationDisplayCount
...
-- PERF: Pre-cached boss unit lookup
local _BOSS_LOOKUP = { boss1=true, boss2=true, boss3=true, boss4=true, boss5=true }
local _BOSS_LABEL = { boss1="Boss 1", boss2="Boss 2", boss3="Boss 3", boss4="Boss 4", boss5="Boss 5" }
...
local function MSUF_A2_PreviewUnitLabel(unit)
    ...
    local bl = _BOSS_LABEL[unit]
    if bl then return bl end
...
local function AcquireIcon(container, index)
    ...
    if icon.cooldown and not icon.cooldown._msufA2_doneHooked then
        ...
        icon.cooldown:SetScript("OnCooldownDone", function(cd)
            ...
            if type(_GetAuraDataByAuraInstanceID) == "function" then
                local auraData = _GetAuraDataByAuraInstanceID(unit, aid)
...
function Apply.CommitIcon(icon, unit, aura, shared, isHelpful, hidePermanent, masterOn, isOwn, stackCountAnchor, layoutSig)
    ...
    then
        ...
        if (showCdText or showCdSwipe) and unit and aid and icon.cooldown then
            ...
            if type(_GetAuraDuration) == "function" then
                local obj = _GetAuraDuration(unit, aid)
            ...
            if not hadTimer then
                ...
                if type(_DoesAuraHaveExpirationTime) == "function" then
                    local v = _DoesAuraHaveExpirationTime(unit, aid)
...
local function MSUF_A2_AuraHasExpiration(unit, aura)
    ...
    if type(_DoesAuraHaveExpirationTime) == "function" then
        local has = _DoesAuraHaveExpirationTime(unit, auraInstanceID)
	...
	if type(_GetAuraDuration) == "function" then
        local d = _GetAuraDuration(unit, auraInstanceID)
...
local function MSUF_A2_AuraIsKnownPermanent(unit, aura)
	...
	if type(_DoesAuraHaveExpirationTime) == "function" then
	    local v = _DoesAuraHaveExpirationTime(unit, auraInstanceID)
	    if not _A2_IsSecretValue(v) then
	        local tv = type(v)
	        	if tv == "boolean" then
...
local function MSUF_A2_TrySetCooldownFromAura(icon, unit, aura, wantCountdownText)
    ...
    if type(_GetAuraDuration) == "function" then
        local obj = _GetAuraDuration(unit, auraInstanceID)
        if obj ~= nil then
            local cd = icon.cooldown
            local applied = false
            if type(cd.SetCooldownFromDurationObject) == "function" then
...
local function MSUF_A2_ApplyIconStacks(icon, unit, shared, stackAnchorOverride, forcedDisp, forceHideCooldownNumbers, allowQuery)
    ...
    if disp ~= nil then
        ...
        if allowQuery ~= false and icon._msufAuraInstanceID then
            ...
            elseif _GetAuraApplicationDisplayCount then
                disp = _GetAuraApplicationDisplayCount(unit, icon._msufAuraInstanceID, 2, 99)
            end
